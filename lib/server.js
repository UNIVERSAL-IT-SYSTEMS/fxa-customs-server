...
module.exports = function createServer(config, log) {
  ...
  // NOTE: we will probably need a new endpoint here just for "checks that are authenticated"
  // something like `checkAuthenticated`? It would block the user if they did something many times
  // such as " tried to upload an avatar 40 times in 15 minutes".
  // I'm not sure if we are going to be using the same `emailRecord, ipRecord, ipEmailRecord` records for storing this yet.
  api.post(
    '/check',
